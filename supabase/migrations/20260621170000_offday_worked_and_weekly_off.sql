-- ════════════════════════════════════════════════════════════════════════════
-- Weekly off (per employee) + worked on a day off, merged with the night shift
-- window logic.
--
-- The night shift migration replaced compute_attendance_for() and, doing so,
-- dropped two things the weekly off migration had added: reading the per
-- employee weekly_off column, and skipping gate only people. This restores both
-- on top of the night shift window, and keeps the rule the client asked for:
-- if a person scans on their day off, that day counts as worked (Present), not
-- as their day off. Their hours are recorded, and they are never marked absent
-- on that day.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- 1) Resolve the timetable + scheduled window for this shift-day FIRST.
  ...
  -- The person's weekly off day has no expected hours, so they are never marked
  -- absent on it. If they DO scan, the branches below still record it as a
  -- worked day (Present, with hours), because has_tt is false here.
  ...
  --    for the day the shift started). Day shift / day off → by local date.
  with src as (
...
-- Repair recent days so the restored weekly off / day off rules show right away.
