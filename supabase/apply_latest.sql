...
--
-- What it does:
--   • Night shifts that start after midnight (e.g. 01:00 to 09:00) now count for
--     the PREVIOUS day, automatically from the hours (no toggle). This clears the
--     false "Absent" rows for the night CSRs.
--   • Editing a timetable or shift now recomputes attendance immediately.
...
-- Column guards: make sure the fields the compute relies on exist, whatever
