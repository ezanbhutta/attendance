...
export default function Shifts() {
  ...
  async function addShift(e) {
    e.preventDefault(); setErr(null);
    const { data, error } = await supabase.from('shifts').insert({ name: shiftName }).select('id').single();
    if (error) return setErr(error);
  ...
  return (
    <>
      <div className="page-title"><h1>Shifts &amp; Timetables</h1></div>
      <ErrorBanner error={err} />

      <Card title="Timetables">
        <form onSubmit={addTt} className="row" style={{ marginBottom: 12 }}>
        ...
        <Table
          ...
          columns={[
            ...
            { key: 'late_grace_min', label: 'Late grace', num: true },
            { key: 'early_leave_grace_min', label: 'Early grace', num: true },
      ...
      <Card title="Shifts">
        <form onSubmit={addShift} className="row" style={{ marginBottom: 12 }}>
      ...
      <Card title="Weekly timetable map" actions={
